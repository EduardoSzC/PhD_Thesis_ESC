...
#-----------------------------
txt_file <- "chamois_ab33_edit_npf.txt"
...
#-----------------------------
# 6. Compute a final score considering feature-side probability
#-----------------------------
#Compute final composite score
final_scored_links <- links_main %>%
  left_join(
    support_scores,
    by = c("FeatureID", "BGC")
  ) %>%
  mutate(
    ...
    FinalScore = (P_bgc_main) * SupportScore
  )
